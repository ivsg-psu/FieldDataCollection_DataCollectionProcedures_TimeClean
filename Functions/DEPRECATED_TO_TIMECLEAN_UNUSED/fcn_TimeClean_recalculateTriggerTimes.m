...
[cell_array_GPS_Time,sensor_names_GPS_Time]         = fcn_LoadRawDataToMATLAB_pullDataFromFieldAcrossAll(dataStructure, 'GPS_Time',sensorType);
...
for ith_sensor = 1:length(sensor_names_GPS_Time)
    ...
    fixedDataStructure.(sensor_name).Trigger_Time = new_Trigger_Time;
    ...
    fixedDataStructure.(sensor_name).GPS_Time = GPS_Time_in_Trigger;
    ...
    sensor_data = fixedDataStructure.(sensor_name);
            ...
            if length(sensor_data.(subFieldName)) ~= 1 % Is it a scalar? If yes, skip it
                ...
                if isequal(size(sensor_data.(subFieldName)),original_vector_size)
                    if strcmp(sensor_name,'LIDAR_Sick_Rear') 
                        ...
                        fixedDataStructure.(sensor_name).(subFieldName) = sensor_data.(subFieldName)(start_index:end_index,:);
                    end
                end
            end
        end
