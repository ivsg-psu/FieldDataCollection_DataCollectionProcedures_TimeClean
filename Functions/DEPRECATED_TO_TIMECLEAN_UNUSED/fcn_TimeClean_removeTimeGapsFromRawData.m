function RawDataWithoutTimeGaps = fcn_TimeClean_removeTimeGapsFromRawData(rawData)
...
for i_data = 1:length(names)
    ...
    if 1==0 % use the rawData.Lidar.scan_time as the time interval, so we do not check lidar's time now
        %copy new time data 
        t_new = d.ROS_Time;
        t_new = t_new(1) + cumsum(d.scan_time) - d.scan_time(1);  % use the first ROS time as the start time and the scan time as interval
        dout.ROS_Time = t_new; 
        if abs(t_new(end)-RawDataWithoutTimeGaps.GPS_Hemisphere.ROS_Time(end))>3*mean(d.scan_time)
            error('Lidar time or GPS_Hemisphere time is wrong')
        end
    else     
        ...
        for i_subField = 1:length(subfieldNames)
            ...
            if any(strcmp(subFieldName,fields_to_calculate_timegaps_for))
                ...
                if flag_do_debug
                    ...
                    fprintf(1,'\t\tStandard deviation in delta_t: %f \n',std_dt);
                    fprintf(1,'\t\tAllowable maximum deviation in delta_t: %f \n',max_dt);
                    fprintf(1,'\t\tAllowable minimum deviation in delta_t: %f \n',min_dt);
                    fprintf(1,'\t\tActual maximum deviation in delta_t: %f ',max(diff_t));
                    if 1==flag_jump_error_detected
                        fprintf(1,' <--FAIL\n');
                    else
                        fprintf(1,' <--Pass\n');
                    end
                    fprintf(1,'\t\tActual minimum deviation in delta_t: %f ',min(diff_t));
                    if 1==flag_jump_error_detected
                        fprintf(1,' <--FAIL\n');
                    else
                        fprintf(1,' <--Pass\n');
