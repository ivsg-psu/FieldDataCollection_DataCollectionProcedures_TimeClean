function fixed_dataStructure = fcn_TimeClean_calculateTriggerTime_Velodyne(dataStructure)
...
%% Find centiSeconds of Velodyne LiDAR
centiSeconds = dataStructure.LiDAR_Velodyne_Rear.centiSeconds;

ROS_Time = dataStructure.LiDAR_Velodyne_Rear.ROS_Time;

cell_array_PointCloud = dataStructure.LiDAR_Velodyne_Rear.PointCloud;

N_scans = dataStructure.LiDAR_Velodyne_Rear.Npoints;
Trigger_time = [];
for idx_scan = 1:N_scans
    current_PointCloud = cell_array_PointCloud{idx_scan};
    time_offets = current_PointCloud(:,6);
    LiDAR_Time = ROS_Time(idx_scan,:) + time_offets;
    Trigger_time(idx_scan,1) = min(LiDAR_Time);
